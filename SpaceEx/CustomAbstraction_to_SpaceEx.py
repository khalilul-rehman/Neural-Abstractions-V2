...
# your StateModel lives here (unchanged)
from CustomAbstraction.CustomAbstractionHelper import StateModel
...
def _ordered_dims(bounds: Dict[int, Tuple[Optional[float], Optional[float]]]) -> List[int]:
    return sorted(bounds.keys())


def _bounds_to_constr(bounds: Dict[int, Tuple[Optional[float], Optional[float]]],
                      var_names: List[str], sep: str = " & ") -> str:
    """
    Build a conjunction of linear bounds, allowing half-spaces.
    For each dimension d in bounds:
      (lo, hi) may be numbers or None.
      - If lo is not None: add 'lo <= x_d'
      - If hi is not None: add 'x_d <= hi'
...
def _u_bounds_sqrt_h(h_scalar: Optional[float], uvars: List[str], sep=" & "):
    """
    Return conjunction of -sqrt(h) <= u_i <= sqrt(h) per i, or "" if h is None/negative.
    """
    if h_scalar is None or h_scalar < 0:
        return ""
    hbar = math.sqrt(h_scalar)
...
def _boxes_intersect(boxA: List[Tuple[Optional[float], Optional[float]]],
                     boxB: List[Tuple[Optional[float], Optional[float]]]) -> bool:
    """
    Axis-aligned intersection test allowing None (meaning -inf / +inf).
    Each list is [(lo,hi), ...] of the same length (or we compare up to min len).
    """
    L = min(len(boxA), len(boxB))
    for i in range(L):
        a_lo, a_hi = boxA[i]
        b_lo, b_hi = boxB[i]
        # map None to infinities
        a_lo = -float('inf') if a_lo is None else a_lo
        b_lo = -float('inf') if b_lo is None else b_lo
        a_hi = float('inf') if a_hi is None else a_hi
        b_hi = float('inf') if b_hi is None else b_hi
        if a_hi < b_lo or b_hi < a_lo:
            return False
    return True
...
def build_invariant_text(state: StateModel, xvars: List[str], uvars: Optional[List[str]],
                         bounded_time: bool, T: float):
    # x-box / half-space constraints
    parts = [_bounds_to_constr(state.bounds, xvars)]
    # u-bounds via sqrt(h) envelope (your chosen semantics)
    if uvars:
        ub = _u_bounds_sqrt_h(state.h, uvars)
        if ub:
            parts.append(ub)
    # time bounds (both sides to avoid unbounded support in t=-1 direction)
    if bounded_time:
        parts.append("0 <= t")
        parts.append(f"t <= {_fmt(T)}")
    return " & ".join(parts)
...
def build_guard_text(src: StateModel, dst: StateModel, xvars: List[str], bounded_time: bool, T: float):
    # target's x-only constraints (+ time cap if bounded)
    g = _bounds_to_constr(dst.bounds, xvars)
    if bounded_time:
        g = g + f" & t <= {_fmt(T)}"
    return g
...
def write_spaceex_xml(
    ...
    out_xml_path: str,
    system_name: str = "NA_like_system",
    bounded_time: bool = True,              # keep NA-like pattern
    T: float = 1.5,
    ...
    n = states[0].M.shape[0]
    xvars = [f"x{i}" for i in range(n)]
    # if any state has h != None, introduce u variables to apply ±sqrt(h)
    use_u = any(s.h is not None for s in states)
    uvars = [f"u{i}" for i in range(n)] if use_u else None
    ...
    if initial_box is not None and init_mode == "init_location":
        init_loc_id = f"loc{L + (1 if bounded_time else 0)}"
        init_flow = " & ".join([f"{xv}'==0" for xv in xvars] + (["t'==1"] if bounded_time else []))
        # keep invariant empty (classic NA pattern)
        body.append(
            f'  <location id="{init_loc_id}" name="Init" x="0" y="{(L//max_cols+1)*y_spacing}" width="{w}" height="{h}">\n'
            f"    <flow>{_xml(init_flow)}</flow>\n"
            f"  </location>\n"
        ...
        # <initially> referencing Init with the initial box
        init_box_dict = {i: (initial_box[i][0], initial_box[i][1]) for i in range(len(initial_box))}
        init_constr = _bounds_to_constr(init_box_dict, xvars)
        init_xml = (
            "  <initially>\n"
            f"    <constr>{_xml(init_constr)}</constr>\n"
            f"    <loc>{init_loc_id}</loc>\n"
    ...
    forb_xml = ""
    if unsafe_box is not None:
        unsafe_dict = {i: (unsafe_box[i][0], unsafe_box[i][1]) for i in range(len(unsafe_box))}
        forb_constr = _bounds_to_constr(unsafe_dict, xvars)
        forb_xml = (
            "  <forbidden>\n"
            f"    <constr>{_xml(forb_constr)}</constr>\n"
            "  </forbidden>\n"
...
def write_spaceex_cfg(
    ...
    system_name: str,
    state_var_names: List[str],
    initial_box: Optional[List[Tuple[Optional[float], Optional[float]]]],
    time_horizon: float,
    scenario: str = "phaver",
    directions: str = "oct",
    sampling_time: float = 0.01,
    ...
    forbidden_box: Optional[List[Tuple[Optional[float], Optional[float]]]] = None,
    set_aggregation: str = "none",
    flowpipe_tolerance: float = 0.01,
    flowpipe_tolerance_rel: float = 0.0,
    rel_err: float = 1e-12,
    abs_err: float = 1e-15,
    output_error: int = 0,
    simu_init_sampling_points: int = 0,
    include_t0_in_initially: bool = True,
):
    """
    Writes a CFG exactly in the sample format you provided:
    - identifiers unquoted (scenario, directions, set-aggregation, output-format)
    - expressions quoted (initially, forbidden, output-variables list)
    """
    def _box_to_constr_list(box: List[Tuple[Optional[float], Optional[float]]]) -> str:
        d = {i: (box[i][0], box[i][1]) for i in range(len(box))}
        return _bounds_to_constr(d, [f"x{i}" for i in range(len(box))], sep=" & ")

    initially = ""
    if initial_box is not None:
        initially = _box_to_constr_list(initial_box)
        if include_t0_in_initially:
            initially += " & t==0"

    forbidden = ""
    if forbidden_box is not None:
        forbidden = _box_to_constr_list(forbidden_box)

    lines = [
        f"system = {system_name}",
        "",
        f'initially = "{initially}"' if initially else 'initially = ""',
    ]
    if forbidden:
    ...
    lines += [
        ...
        "",
        f"set-aggregation = {set_aggregation}",
        f"flowpipe-tolerance = {flowpipe_tolerance}",
        f"flowpipe-tolerance-rel = {flowpipe_tolerance_rel}",
        f"rel-err = {rel_err}",
        f"abs-err = {abs_err}",
        f"output-error = {output_error}",
        f"iter-max = {iter_max}",
        f"simu-init-sampling-points = {simu_init_sampling_points}",
        "",
        f"time-horizon = {time_horizon}",
        f"sampling-time = {sampling_time}",
        "",
        f'output-variables = "{", ".join(state_var_names)}"',
...
def export_spaceex(
    ...
    system_name: str = "NA_like_system",
    T: float = 1.5,
    initial_box: Optional[List[Tuple[Optional[float], Optional[float]]]] = None,
    unsafe_box: Optional[List[Tuple[Optional[float], Optional[float]]]] = None,
    init_mode: str = "init_location",  # "init_location" (NA-like) or "global_initially"
    ...
    write_spaceex_cfg(
        ...
        directions=cfg_directions,
        sampling_time=cfg_sampling_time,
        iter_max=cfg_iter_max,
        output_format=cfg_output_format,
        forbidden_box=unsafe_box,
        set_aggregation="none",
        flowpipe_tolerance=0.01,
        flowpipe_tolerance_rel=0.0,
        rel_err=1e-12,
        abs_err=1e-15,
        output_error=0,
        simu_init_sampling_points=0,
        include_t0_in_initially=True
    )
    return xml_path, cfg_path
